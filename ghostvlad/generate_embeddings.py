...
def prepare_data(SRC_PATH):
    ...
    for i,spkDir in enumerate(wavDir):   # Each speaker's directory
        ...
        wavPathDir = os.listdir(wavPath)

        # folder inside speaker folder
        for j,showDir in enumerate(wavPathDir):
            wavFilePath = os.path.join(wavPath, showDir)

            #for wav in os.listdir(wavPath): # wavfile
                #utter_path = os.path.join(wavPath, wav)
            for wav in os.listdir(wavFilePath): # wavfile
                utter_path = os.path.join(wavFilePath, wav)
        # end of renni
...
def main():
    ...
    for epoch in range(70): # Random choice utterances from whole wavfiles
    ...
    print("time needed: {}".format(time_end - time_start))
    # end of renni
